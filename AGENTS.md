...
The current environment observation is controlled by `EnvConfig.observation_mode` and always remains a 9-dimensional vector. In the default `calendar` mode it is ordered as: `is_weekday_day0`, `is_weekday_day1`, `is_weekday_day2`, `inventory_day0`, `inventory_day1`, `inventory_day2`, `reference_price_day0`, `reference_price_day1`, `reference_price_day2`. In `forecast` mode, the first three dimensions are replaced by `expected_arrivals_day0`, `expected_arrivals_day1`, and `expected_arrivals_day2`, computed from calibrated weekday/weekend arrival means and `ideal_offset_probs` before daily demand is sampled. This is an explicit exogenous demand-forecast prior, not realized-demand leakage. Policies or baselines that read raw observations should use `obs[0:3]` for either the configured demand/calendar features, `obs[3:6]` for inventory, and `obs[6:9]` for reference prices.
...
The environment uses a single raw reward definition: contribution profit minus weighted scarcity penalty. The value returned to RL algorithms is `raw_reward / EnvConfig.reward_scale`, with `reward_scale=1000.0` by default. Keep `raw_reward`, scaled `reward`, contribution-profit `revenue`, total penalty, and scarcity penalty logged separately so experimental comparisons remain interpretable. Set `EnvConfig.scarcity_penalty_coef=0.0` for the no-scarcity-penalty case. PPO keeps online reward normalization enabled by default; SAC, TD3, and TQC default to fixed reward scaling with online reward normalization disabled.
